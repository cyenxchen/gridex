...
```bash
cd /e/dev/be/vura
powershell -NoProfile -ExecutionPolicy Bypass -File ./windows/scripts/build-and-pack.ps1 -Version "0.1.6"
```
...
2. Build `Gridex.exe` unpackaged, stamp version `0.1.6` vào About section
3. Pack thành `Setup.exe` qua Velopack với metadata version `0.1.6`

**Output**: `windows/Releases/`
- `Gridex-stable-Setup.exe` — installer user-facing
- `Gridex-0.1.6-stable-full.nupkg` — package Velopack
...
powershell -NoProfile -ExecutionPolicy Bypass -File ./windows/scripts/build-unpackaged.ps1 -Version "0.1.6-test"
```
...
**Git Bash / MSYS2**:
```bash
powershell -NoProfile -ExecutionPolicy Bypass -File ./windows/scripts/pack-velopack.ps1 -Version "0.1.6-test"
```
...
- `windows/Gridex/GridexVersion.generated.h` — **auto-generated, gitignored**, do `build-unpackaged.ps1` ghi mỗi lần chạy
  - Có `-Version` → `#define GRIDEX_VERSION L"0.1.6"`
...
- CI workflow `.github/workflows/windows-release.yml` tự parse version từ git tag (`v0.1.6` → `0.1.6`) và forward sang script
...
**Git Bash / MSYS2**:
```bash
powershell -NoProfile -ExecutionPolicy Bypass -File ./windows/scripts/build-and-pack.ps1 -Version "0.1.6-beta.1" -Channel "beta"
```
...
### Setup.exe cài xong nhưng About hiện `0.0.0-dev` dù pack với `-Version 0.1.6`
